...
> **⚠️ BREAKING CHANGE**: Starting from version 2.0.0, this exporter no longer automatically creates tables. Tables must be pre-created before using the exporter. See the [Migration Guide](#migration-guide) for upgrade instructions.
...
**Important**: Before using this exporter, you must create the necessary tables in S3 Tables. The exporter does not automatically create tables - they must be pre-created with the appropriate Iceberg schemas.

### Creating Tables

Use the AWS CLI to create tables for each telemetry type you plan to export:
...
**Note**: You only need to create tables for the telemetry types you plan to use. For example, if you only export metrics, you only need to create the metrics table.
...
**Note**:
- The `s3tables:GetTable` permission is required to retrieve table information (warehouse location and version token)
- The `s3:PutObject` permission is required for the warehouse location buckets, which follow the pattern `*--table-s3`. These are automatically generated by S3 Tables for each table
- Table creation permissions (`s3tables:CreateTable`, `s3tables:CreateNamespace`) are not required as tables must be pre-created before using the exporter

## Troubleshooting

### Common Errors and Solutions

#### Error: "table not found" or "failed to get table information"

**Cause**: The specified table does not exist in the namespace.

**Solutions**:
1. Verify the table exists in your S3 Tables bucket:
   ```bash
   aws s3tables list-tables \
     ...
     --region us-east-1
   ```
2. Create the table if it doesn't exist (see [Prerequisites](#prerequisites) section for table creation commands)
3. Verify the `namespace` and table names in your configuration match the actual table names
4. Check that the namespace exists:
   ```bash
   aws s3tables list-namespaces \
     --table-bucket-arn "arn:aws:s3tables:us-east-1:123456789012:bucket/my-table-bucket" \
     --region us-east-1
   ```
5. If the namespace doesn't exist, create it first before creating tables
...
Version 2.0.0 introduces a breaking change: the exporter no longer automatically creates tables. This change simplifies the exporter's responsibilities and provides better control over table management.

#### What Changed

- **Removed**: Automatic table creation functionality
- **Removed**: Automatic namespace creation functionality
- **Removed**: IAM permissions for `s3tables:CreateTable` and `s3tables:CreateNamespace`
- **Required**: Tables must be pre-created before starting the exporter

#### Migration Steps

1. **Identify your current tables**: Check which tables are currently being used by your exporter configuration

2. **Create tables manually**: Use the AWS CLI to create all necessary tables (see [Prerequisites](#prerequisites) section)
   ```bash
   # Example: Create metrics table
   aws s3tables create-table \
     --table-bucket-arn "arn:aws:s3tables:us-east-1:123456789012:bucket/my-table-bucket" \
     --namespace "telemetry" \
     --name "otel_metrics" \
...
3. **Update IAM permissions**: Remove `s3tables:CreateTable` and `s3tables:CreateNamespace` permissions from your IAM policy. Only `s3tables:GetTable` and `s3:PutObject` are now required (see [IAM Permissions](#iam-permissions) section)

4. **Test the configuration**: Start the exporter and verify it can successfully retrieve table information and write data

5. **Monitor for errors**: Check logs for any "table not found" errors, which indicate missing tables

#### Rollback Plan

If you need to rollback to v1.x:
1. Downgrade the exporter to the previous version
2. Restore the previous IAM permissions (including `s3tables:CreateTable` and `s3tables:CreateNamespace`)
3. The exporter will resume automatic table creation

#### Benefits of This Change

- **Clearer separation of concerns**: Table management is separate from data writing
- **Better control**: Tables can be created with specific configurations using IaC tools
- **Reduced permissions**: The exporter requires fewer IAM permissions
- **Improved security**: Principle of least privilege - the exporter only needs read and write permissions